...
const path = require('path'); // 提前引入path模块，解决顺序错误
const app = express();
...
// CORS 配置 - 允许所有来源（开发环境）
// 区分开发/生产环境的CORS配置
const allowedOrigins = currentConfig.mode === 'mock'
    ? ['*'] // 开发环境允许所有
    : ['https://your-domain.com', 'https://admin.your-domain.com']; // 线上域名白名单
...
app.use(cors({
    ...
    credentials: currentConfig.mode !== 'mock' // 生产环境启用Credentials
}));
app.use(express.json());
app.use(express.static(path.join(__dirname, 'static'))); // 提供static目录的静态文件

// gateway.js 新增代理配置
const { createProxyMiddleware } = require('http-proxy-middleware'); // 先引入代理模块
...
// 后端代理配置（转发所有/api请求到PyCharm后端）
const backendProxy = createProxyMiddleware({
  target: 'http://localhost:5000',
  pathFilter: '/proxy/api/**', // 仅匹配带/proxy前缀的/api请求
  changeOrigin: true,
  pathRewrite: { '^/proxy/api': '/api' }, // 转发时移除/proxy前缀（后端仍接收/api请求）
  onProxyReq: (proxyReq, req, res) => {
    console.log(`🔄 [网关转发] ${req.method} ${req.path} -> http://localhost:5000${req.path.replace('/proxy', '')}`);
  },
  onError: (err, req, res) => {
    res.status(502).json({
      success: false,
      message: `网关转发失败：无法连接到后端 ${err.message}`
    });
  }
});
app.use(backendProxy); // 保持在请求日志中间件之后、404之前
